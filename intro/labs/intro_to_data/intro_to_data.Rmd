...
work with a small subset. You can load the data set (and the packages we'll use)
with
...
variable that we want to visualize, `age`, so we don't fill in the `y =` 
...
### Summary statistics through a data pipeline

A very useful paradigm for working with data is to sketch out a data pipeline.
This should always start with the original data frame and usually ends with 
numerical summaries, plots, or a newly modified data frame.

As a simple example, lets create a pipeline (also called a chain) that returns 
some information about the `weight` variable. We'll use the function `summary()`,
which returns a numerical summary including the minimum, first quartile, median,
...
The `%>%` operator is called the **piping** operator and all it does is take the 
output of the current line and pipes it into the following line of code. It can 
be read as "then", so these three lines of code could be read: "Take the `cdc` 
data set then select the `weight` variable then summarize it."

You can also individually calculate summary statistics, and make your own customized list
of them using the `summarize` function:

```{r summary-custom-list}
cdc %>%
  summarize(mean_wt = mean(weight), sd_wt = sd(weight), mean_ht = mean(height), sd_ht = sd(height))
```

Note that in the `summarize` function we created a list of four elements. The names
...
2.  Calculate the median weights of people who did and did not exercise in the 
last month.
...
We can visualize these counts by constructing a bar plot.
...
Note that because we specified a categorical variable to be on the x-axis, each
category has its own boxplot for the `height` of the people within that category.
...
Here the output of our data pipeline is a new dataset called `males`. The special
symbol `<-` performs an *assignment*, taking the output of one line of code and 
saving it into this new object that you defined. If you had run the data pipeline
without the assignment, it would simply have printed the filtered dataset to 
your console instead of capturing it for future use.
...
essential computing tools -- summary statistics, subsetting, plots, and the idea
of a data pipeline -- that will serve us well throughout this course.
