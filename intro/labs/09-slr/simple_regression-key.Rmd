...
*The smallest sum of squares that I got was 129750.5, but my neighbor was able
to get ~126,000.*
...
```{r ex4, eval = TRUE, echo = FALSE}
m1 <- lm(runs ~ homeruns, data = mlb11)
summary(m1)
```

$$ \widehat{runs} = 415.2389 + 1.8345 \times homeruns $$

*The slope tells us that we'd expect a team that scores one more homerun in a
season to score about 1.8 more home runs, on average.*
...
```{r ex5, eval = TRUE, echo = TRUE}
m1 <- lm(runs ~ at_bats, data = mlb11)
y_hat <- -2789.2429 + .6305 * 5579
```

*Based on our regression model, we could predict that a team with 5,579 at bats
would score 728.3166 runs in a season. In our data set, the Philadelpha Phillies
had 5,579 at bats, so their residual indicates that they underperformed (our
...
former has an $R^2$ value of `r rsq1` and the latter has an $R^2$ of `r rsq2`.*
