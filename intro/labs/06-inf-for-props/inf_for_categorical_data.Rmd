...
The inferential tools for estimating population proportion are the two that we've
become familiar with by now: the confidence interval and the 
hypothesis test.
...
inferences. For inference on proportions, the sample proportion can be assumed 
...
10. Repeat the above simulation three more times but with modified sample
    ...
    distributions (either histograms or density plots). Based on these limited plots, how does $n$ appear to affect the 
